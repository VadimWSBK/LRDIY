...
class CaravanRoofCalculator extends HTMLElement {
    ...
    calculateProductPrice(product, length, width, roofType) {
        ...
        if (product.name === 'Waterproof Sealant') {
            const coverage = product.coveragePerLitre;
            return Math.ceil(area / coverage) * product.buckets[0].price; // Adjust based on your logic
        }

        // Add similar logic for Thermal Coating and Primer
        return 0; // Default
    }
}
