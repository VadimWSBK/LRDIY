...
// Function to update the primer product based on roof type
    function updatePrimerProduct(roofType) {
        let primerProduct = {
    ...
    function showInfoPopup(linkElement, popupText) {
    ...
    document.getElementById('caravan-roof-type').addEventListener('change', function () {
        ...
        updatePrimerProduct(roofType); // Update primer product
        displayProducts(length, width, roofType);
    });
    ...
    document.getElementById('caravan-length').addEventListener('input', function () {
        this.value = enforceMaxValue(this.value, 20);
        });

        document.getElementById('caravan-width').addEventListener('input', function () {
         this.value = enforceMaxValue(this.value, 3);
        });

        // Function to enforce maximum values
        function enforceMaxValue(value, max, elementId) {
    ...
    let globalProductSelections = []; // To store product selections for the Buy Now button

    function displayProducts(length = 6, width = 2.5, roofType = 'painted') {
    ...
    productCheckbox.addEventListener('change', function () {
        const priceForProduct = calculateProductPrice(product, length, width, roofType);

        // BONUS product check/uncheck logic
        if (product.name === 'BONUS') {
            if (this.checked) {
                // Ensure there's at least one other product selected
                if (globalProductSelections.length === 0 || globalProductSelections.every(p => p.variantId === product.variants[0].variantId)) {
                    showCustomAlert('You must select at least one other product before adding the BONUS product.');
                    this.checked = false;
                    return;
                }
                addProductToSelections(product, length, width, roofType);
            } else {
                removeProductFromSelections(product);
            }
        } else {
            // Normal product logic for check/uncheck
            if (this.checked) {
        ...
        infoLink.addEventListener('click', function () {
            showInfoPopup(this, product.infoText);
        });

        const productQuantity = document.createElement('div');
        productQuantity.classList.add('caravan-product-quantity');
        const bucketSizes = calculateBucketSizes(product, length, width, roofType);
    ...
    function calculateProductPrice(product, length, width, roofType) {
    const area = length * width;
    ...
    if (product.coveragePerLitre && product.buckets) {
        ...
        // Adjust the bucket counts according to the rules
...
function calculateBucketSizes(product, length, width, roofType) {
    const area = length * width;
    let buckets = [];
    ...
    if (product.name === 'Geo Textile') {
        ...
        // Directly push the BONUS variant
        const variant = product.variants.find(v => v.variantId === '40918670606469'); // Adjust if needed
        if (variant) {
            buckets.push(`1 x ${variant.name}`);
        }
        ...
        // Find best bucket combination
        const bestCombination = findBestBucketCombination(requiredVolume, product.buckets);

        // Adjust the bucket counts according to the rules
        const adjustedCombination = adjustBucketCounts(bestCombination, product.buckets, product);

        // Convert adjustedCombination.buckets to an array of strings
    ...
    return buckets; // Ensure to return the buckets array
}



    function findBestBucketCombination(requiredVolume, buckets) {
        let minTotalCost = Infinity;
        let minWaste = Infinity;
        let bestCombination = null;

        // Limit the number of buckets to reasonable amounts
    ...
    // Updated adjustBucketCounts function
    function adjustBucketCounts(bestCombination, availableBuckets, product) {
        // bestCombination.buckets is an array of {size, count, price}
        // availableBuckets is an array of {size, price, variantId}
        ...
        if (product.name === 'Sealer/Primer' || product.name === 'Etch Primer') {
            // For primer products, allow 2 x 1L buckets
            if ((bucketCounts[1] || 0) >= 3) {
                bucketCounts[4] = (bucketCounts[4] || 0) + 1;
                bucketCounts[1] = 0;
            }
            // Apply the standard rule for 4L buckets
            if ((bucketCounts[4] || 0) >= 3) {
                bucketCounts[15] = (bucketCounts[15] || 0) + 1;
                bucketCounts[4] = 0;
            }
        } else {
            // For other products, apply the original rules
        ...
        // Reconstruct the buckets array and recalculate total cost
        let adjustedBuckets = [];
        let totalCost = 0;
...
// ADD Product to SELECTION
        ...
        function addProductToSelections(product, length, width, roofType) {
            const area = length * width;
            ...
            if (product.name === 'Geo Textile') {
                ...
                // Ensure BONUS product isn't added unless another product is already selected
                if (globalProductSelections.length === 0 || globalProductSelections.every(p => p.variantId === product.variants[0].variantId)) {
                    showCustomAlert('You must select at least one product before adding the BONUS product.');
                    this.checked = false; // Uncheck the checkbox since no other product is selected
                    return;
                }
                globalProductSelections.push({
        ...
        function removeProductFromSelections(product) {
            if (product.name === 'BONUS') {
                // Remove BONUS product from global selections
...
// Event listener for Buy Now button
        document.getElementById('caravan-buy-now-button').addEventListener('click', function () {
            ...
            // Add discount code
            const discountCode = 'Caravan-KIT-10OFF-Offer'; // Replace with your actual discount code
            const discountParameter = `discount=${encodeURIComponent(discountCode)}`;

            // Create the checkout URL with UTM parameters and discount code
